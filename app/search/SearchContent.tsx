    ...
    <div className="container mx-auto max-w-7xl px-4 py-8">
      ...
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-white/45">
          Cari series, translator, creator, user,
          atau community di Ellaria.
        </p>
      </div>
          ...
          {type === 'series' || type === 'all' ? (
            <>
              ...
              <select
                ...
                className="h-10 max-w-[220px] rounded-xl border border-white/10 bg-[#111820] px-3 text-sm text-white outline-none focus:border-[#42A5F5]/50"
              >
                <option value="">
                  {genresLoading
                    ? 'Memuat genre...'
        ...
        {showFilters && (
          <div className="mt-3 border-t border-white/10 pt-3">
            <div className="text-xs leading-5 text-white/40">
              ...
              Pilih genre dari daftar untuk
              mempersempit hasil series.
            </div>
          </div>
        )}
      </div>
      ...
      {!loading && !hasResults && (
        <div className="rounded-2xl border border-white/10 bg-[#0b1016] px-6 py-16 text-center">
          ...
          <p className="mt-2 text-sm text-white/40">
            Coba kata kunci atau filter yang
            berbeda.
          </p>
        </div>
      )}
...
function FilterSelect({
  ...
  return (
    <select
      ...
      className="h-10 rounded-xl border border-white/10 bg-[#111820] px-3 text-sm text-white outline-none focus:border-[#42A5F5]/50 disabled:opacity-50"
    >
      {options.map((option) => (
        <option
          key={option.value}
      ...
      <div className="p-3">
        ...
        {item.genres.length > 0 && (
          <div className="mt-2 flex gap-1 overflow-hidden">
            {item.genres.slice(0, 2).map((itemGenre) => (
              <span
                key={itemGenre.id}
...
function formatSeriesType(type: string) {
  switch (type) {
    case 'MANGA':
      return 'Manga'
    case 'MANHWA':
