...
async function getStreak(userId: string): Promise<number> {
  ...
  for (let i = 0; i < claims.length - 1; i++) {
    ...
    const previous = new Date(claims[i + 1].claimedAt)
...
export async function POST(req: Request) {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const lastClaim = await tx.userDailyReward.findFirst({
        where: {
          userId: session.user.id,
        },
        orderBy: {
      ...
      if (lastClaim) {
        ...
        if (lastDate.getTime() === yesterday.getTime()) {
          const claims = await tx.userDailyReward.findMany({
            where: {
              userId: session.user.id,
            },
            orderBy: {
