...
export async function GET() {
  try {
    ...
    console.error(error)
    return NextResponse.json({ success: false, message: 'Gagal mengambil genre' }, { status: 500 })
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    if (!name || !slug) return NextResponse.json({ success: false, message: 'Nama genre wajib diisi' }, { status: 400 })

    const genre = await prisma.genre.create({ data: { name, slug } })
    return NextResponse.json({ success: true, data: genre })
  } catch (error) {
    console.error(error)
    return NextResponse.json({ success: false, message: 'Gagal membuat genre' }, { status: 500 })
  }
}
